...
fic_error_t OnewireBus::find_device(uint64_t address, uint64_t address_mask) {
    int device_num = 0;
...
fic_error_t OnewireBus::reset() {
    return (onewire_bus_reset(bus) == ESP_OK) ? FIC_OK : FIC_ERR_SDK_FAIL;
}

fic_error_t OnewireBus::read_bytes(std::vector<uint8_t> &rx_buf) {
